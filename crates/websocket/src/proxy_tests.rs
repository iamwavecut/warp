...
fn no_proxy_suffix_with_dot() {
    ...
    unsafe { env::set_var("HTTPS_PROXY", "http://proxy:3128") };
    // FIXME: Audit that the environment access only happens in single-threaded code.
    unsafe { env::set_var("NO_PROXY", ".example.test") };

    assert!(resolved_proxy_tls("sessions.app.example.test").is_none());

    assert!(resolved_proxy_tls("example.test").is_some()); // Exact "example.test" != ".example.test"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.example.test" ends with ".example.test" and matches.
    assert!(resolved_proxy_tls("sessions.app.example.test").is_none());
    // Exact match too
    assert!(resolved_proxy_tls("example.test").is_none());
    assert!(resolved_proxy_tls("not-example.test").is_some());
    clear_proxy_env();
}
