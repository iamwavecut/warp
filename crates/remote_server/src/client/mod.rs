...
pub struct RemoteServerClient {
    ...
    /// Dedicated channel for `RequestFailed` notifications. Separate from
...
impl RemoteServerClient {
    ...
    /// [`RequestFailedEvent`] notification on error, so transport-level
    /// failures are tracked without requiring each caller
