...
use crate::proto::{
    ...
    UnsubscribeDiffState, notification, server_message, session_scoped_request,
};
...
pub struct RemoteServerClient {
    ...
    /// Dedicated channel for `RequestFailed` diagnostic events. Separate from
...
impl RemoteServerClient {
    ...
    /// fire a `RequestFailed` diagnostic event. Returning `false` does
    ...
    /// failures are tracked for diagnostics without requiring each caller
...
/// in a shared buffer for diagnostics.
#[cfg(not(target_family = "wasm"))]
