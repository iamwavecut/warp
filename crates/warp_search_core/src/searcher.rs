...
impl SearcherWriterWrapper {
    ...
    fn execute_operations(
        ...
        if let Err(e) = writer.commit() {
            log::error!(
                "{:?}",
                anyhow::Error::new(e).context("Failed to commit index writer")
            );
            writer.rollback()?;
    ...
    while running {
        ...
        if batch.is_empty() && rebuild.is_none() {
                ...
                && let Err(e) = writer_handle.lock().wait_on_and_drop_indexing_threads()
            {
                log::error!(
                    "{:?}",
                    e.context("Failed to wait on Tantivy indexing threads")
                );
            }
            continue;
        }
        ...
        for events in merge_with_rebuild(batch, rebuild) {
            if let Err(e) = writer_handle.lock().execute_operations(events) {
                log::error!("{:?}", e.context("Failed to execute search events"));
            }
        }
    }
}
