...
fn wss_becomes_https_and_strips_path() {
    let got = derive_http_origin_from_ws_url("wss://localhost:8080/graphql/v2");
    assert_eq!(got.as_deref(), Some("https://localhost:8080"));
}
...
fn unparseable_input_returns_none() {
    ...
    assert!(derive_http_origin_from_ws_url("https://localhost:8080").is_none());
}
