...
Let's say we are interested in parsing the input `warp --disable-smartmode`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "warp --disable_smartmode";
let start_offset = 0;
...
```rust
let input = "warp --disable-smartmode";
...
assert_eq!(word3.slice(input), "--disable-smartmode");
```
...
let example = "warp -p --disable-smartmode";
...
let expected_word2 = String::from("--disable-smartmode").spanned(Span::new(5,24));
