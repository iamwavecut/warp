...
impl VoiceInput {
    ...
    pub fn start_listening(
        ...
        // error level on every invocation floods logs/error-reporting backends
        // with identical events, so only report the first error per session at
        // error level and downgrade the rest to debug.
