    ...
    fn try_from(result: UploadArtifactResult) -> Result<Self, Self::Error> {
        match result {
            UploadArtifactResult::Success {
                ...
                size_bytes,
                ..
            } => Ok(Self::UploadFileArtifact(api::UploadFileArtifactResult {
    ...
    fn try_from(result: RunAgentsResult) -> Result<Self, Self::Error> {
        match result {
            RunAgentsResult::Launched {
                ...
                let harness = build_api_harness(&harness_type);
                let execution_mode = match execution_mode {
                    RunAgentsLaunchedExecutionMode::Local => Some(api::run_agents::ExecutionMode {
                        mode: Some(api::run_agents::execution_mode::Mode::Local(
                            api::run_agents::Local {},
                        )),
                    }),
                    // Remote orchestration is not a product surface in this fork.
                    RunAgentsLaunchedExecutionMode::Remote { .. } => None,
                };
                let agents = agents
                    .into_iter()
                    .map(|outcome| {
                        let mut outcome: api::run_agents_result::AgentOutcome = outcome.into();
                        outcome.model_id.clone_from(&model_id);
                        outcome.harness = harness.clone();
                        outcome.execution_mode = execution_mode.clone();
                        outcome
                    })
                    .collect();
