//! Explicit local error logging for Warp.
//!
...
//! that depends only on third-party crates, so any crate in the workspace can classify and log
//! errors without creating a dependency cycle with `warp_core`.
//!
//! Errors reported via these macros are local diagnostics only in this fork.
...
/// level. This fork keeps these diagnostics local and does not upload them to a remote incident
/// service.
#[macro_export]
macro_rules! report_error {
    ...
    // Reports `err` and appends structured fields to the local log line.
...
/// Runs `report` with optional structured fields.
#[doc(hidden)]
