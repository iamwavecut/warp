...
The selected value reflects the active execution profile's current `ask_user_question` permission. Selecting an option immediately updates the active profile, hides the footer on the current card, and leaves the local one-shot flag consumed.
### Overlay behavior
