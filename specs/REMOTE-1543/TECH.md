...
- `app/src/terminal/view/queued_prompts_panel.rs` owns panel rendering and row-level interactions: collapse, edit, delete, and drag reorder. The panel looks up the active conversation for its terminal view via `BlocklistAIHistoryModel` rather than duplicating that state.
...
- `QueuedQueryOrigin` distinguishes `/queue` rows from auto-queue rows for UI behavior without affecting firing semantics.
...
### Local Diagnostics
Panel-only interactions are handled locally in `QueuedPromptsPanelView`. This fork does not add product telemetry for collapse, edit, delete, or reorder actions.
## End-to-end flow
```mermaid
flowchart LR
...
- Behavior 44: verify that queue-panel interactions remain local UI state and do not add product telemetry hooks.
