...
`skills-lock.json` is the standard project lock file managed by `npx skills`. In this fork, common skill installation is explicit: run `./script/bootstrap --install-common-skills`, `./script/run --install-common-skills`, or `./script/install_common_skills --if-needed` when you want to refresh the checkout-local skill stamp. To update the locked common skills, run `npx --yes skills@1.5.6 update -p -y` and commit the resulting `skills-lock.json` and `.agents/skills` changes.
