...
use crate::ai::execution_profiles::{
    AIExecutionProfile, ActionPermission, WriteToPtyPermission, long_context_pricing_warning_title,
};
...
impl AgentsWidget {
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let codebase_context_description = vec![FormattedTextFragment::plain_text(
            "Allow the Warp Agent to generate an outline of your codebase that can be used for local context.",
        )];
        ...
        if FeatureFlag::AgentView.is_enabled() {
            static AUTODETECTION_DESCRIPTION_FRAGMENTS: LazyLock<Vec<FormattedTextFragment>> =
                LazyLock::new(|| {
                    vec![FormattedTextFragment::plain_text(
                        "Agent prompt autodetection runs locally and can be adjusted with the denylist below.",
                    )]
                });
            ...
            > = LazyLock::new(|| {
                vec![
                    ...
                    FormattedTextFragment::plain_text(
                        " Detection runs locally and can be adjusted with the denylist below.",
                    ),
                ]
            });
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let mcp_description = vec![FormattedTextFragment::plain_text(
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn search_terms(&self) -> &str {
        "oz agent workspace computer use"
    }
    ...
    fn render(
        ...
        let AgentComputerUseState {
            enabled: is_checked,
            is_forced_by_org,
        } = ComputerUsePermission::resolve_agent_state(app);
        ...
        let toggle_row = build_toggle_element(
            render_body_item_label::<AISettingsPageAction>(
                "Computer use in agent workspaces".to_string(),
        ...
        Flex::column()
            ...
            .with_child(toggle_row)
            .with_child(render_ai_setting_description(
                "Enable computer use in local agent workspace conversations started from the app.",
                !is_disabled,
                app,
            ))
            .finish()
...
fn custom_provider_api_key_update(
    direct_key: &str,
    initial_api_key_fingerprint: Option<u64>,
) -> Option<Option<String>> {
    let direct_key = direct_key.trim();
    let current_fingerprint = (!direct_key.is_empty()).then(|| api_key_fingerprint(direct_key));
    if current_fingerprint == initial_api_key_fingerprint {
        return None;
    }
...
fn custom_provider_ids_are_persisted(providers: &[CustomProviderConfig]) -> bool {
    let mut ids = HashSet::new();
    providers.iter().all(|provider| {
        provider
            .local_id
            .as_ref()
            .is_some_and(|local_id| !local_id.is_empty() && ids.insert(local_id.clone()))
    })
}

fn custom_provider_editor_actions_allowed(keys_ready: bool, provider_ids_persisted: bool) -> bool {
    keys_ready && provider_ids_persisted
}
...
fn normalize_custom_provider_ids(providers: &mut [CustomProviderConfig]) -> bool {
    let mut seen = HashSet::new();
    let mut changed = false;

    for provider in providers {
        let has_unique_id = provider
            .local_id
            .as_ref()
            .is_some_and(|local_id| !local_id.is_empty() && seen.insert(local_id.clone()));
        if has_unique_id {
            continue;
        }

        let mut local_id = new_custom_provider_id();
        while !seen.insert(local_id.clone()) {
            local_id = new_custom_provider_id();
        }
        provider.local_id = Some(local_id);
        changed = true;
    }
...
fn ensure_custom_provider_ids(
    ctx: &mut ViewContext<AISettingsPageView>,
) -> (Vec<CustomProviderConfig>, bool) {
    let mut providers = AISettings::as_ref(ctx).custom_providers.clone();
    if normalize_custom_provider_ids(&mut providers) {
        return match persist_custom_provider_configs(providers, ctx) {
            Ok(providers) => (providers, true),
            Err(error) => {
                report_error!(error);
                (AISettings::as_ref(ctx).custom_providers.clone(), false)
            }
        };
    }
    (providers, true)
...
fn persist_custom_provider_configs(
    mut configs: Vec<CustomProviderConfig>,
    ctx: &mut ViewContext<AISettingsPageView>,
) -> anyhow::Result<Vec<CustomProviderConfig>> {
    normalize_custom_provider_ids(&mut configs);
    AISettings::handle(ctx).update(ctx, |settings, ctx| {
        settings.custom_providers.set_value(configs.clone(), ctx)
    })?;
...
fn merge_provider_editor_config_with_live(
    ...
    edited.local_id = live.local_id.clone();
...
fn find_live_provider_index(
    ...
    live_providers
        .iter()
        .position(|existing| existing.local_id.as_deref() == Some(provider_id))
}
...
fn sync_llm_provider_editors_to_settings(
    ...
    if !custom_provider_ids_are_persisted(&live_providers) {
        return;
    }
    ...
    for provider in providers {
        ...
        let Some(live_index) = find_live_provider_index(&live_providers, &provider.provider_id)
        else {
            // The editor belongs to an older page instance. If its provider was
            // removed or replaced since then, do not apply stale edits to another
            // provider that happens to reuse the old display name.
            continue;
        };
...
impl LLMProvidersWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let provider_editing_enabled =
            custom_provider_editor_actions_allowed(provider_keys_ready, provider_ids_persisted);
        ...
        let mut editor_handles = providers
            ...
            .map(|(_, provider)| {
                ...
                let initial_api_key = stored_keys.get(&provider.name).cloned().unwrap_or_default();
                let api_key_editor =
                    create_llm_provider_editor(initial_api_key.clone(), "sk-...", true, false, ctx);
        ...
        add_provider_button.update(ctx, |button, ctx| {
            button.set_disabled(!provider_editing_enabled, ctx);
        });
...
impl SettingsWidget for LLMProvidersWidget {
    ...
    fn render(
        ...
        } else if !self.provider_ids_persisted {
            content.add_child(
                Text::new_inline(
                    "Provider editing is disabled because local settings could not be saved.",
                    appearance.ui_font_family(),
                    CONTENT_FONT_SIZE,
                )
                .with_color(appearance.theme().disabled_ui_text_color().into())
