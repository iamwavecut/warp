            ...
            ToggleWarpCompletions => {
                InputSettings::handle(ctx).update(ctx, |input_settings, ctx| {
                    if let Err(error) = input_settings
                        ...
                        .toggle_and_save_value(ctx)
                    {
                        log::error!("Failed to save Warp completions setting: {error:?}");
                    }
                });
            }
            ToggleNativeShellCompletions => {
                InputSettings::handle(ctx).update(ctx, |input_settings, ctx| {
                    if let Err(error) = input_settings
                        ...
                        .toggle_and_save_value(ctx)
                    {
                        log::error!("Failed to save native shell completions setting: {error:?}");
                    }
                });
            }
            ToggleAiCommandSearchHashTrigger => {
...
impl SettingsWidget for ConfirmCloseSharedSessionWidget {
    ...
    fn search_terms(&self) -> &str {
        "warning popup modal dialog hosted session close"
    }
    ...
    fn render(
        ...
        let session_settings = SessionSettings::as_ref(app);
        render_body_item::<FeaturesPageAction>(
            "Confirm before closing hosted session".into(),
            None,
            LocalOnlyIconState::for_setting(
