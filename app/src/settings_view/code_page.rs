...
use warp_util::{local_or_remote_path::LocalOrRemotePath, path::user_friendly_path};
use warpui::{
...
const CODEBASE_INDEX_DESCRIPTION: &str = "WarpOss can automatically index code repositories into the local SQLite store using your configured OpenAI-compatible embeddings endpoint. Source chunks are sent only to that endpoint; if embeddings are unavailable, agents continue with local lexical search, grep, and find.";
...
const AUTO_INDEX_DESCRIPTION: &str = "When enabled, WarpOss locally indexes repositories as you navigate them using the configured embeddings provider.";
