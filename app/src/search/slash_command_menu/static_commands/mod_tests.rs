...
// --- AMBIENT_AGENT_V2 flag tests ---
#[test]
fn ambient_agent_v2_required_command_satisfied_in_v2_session() {
    let command =
        Availability::AGENT_VIEW | Availability::AI_ENABLED | Availability::AMBIENT_AGENT_V2;

    // V2 ambient-agent composing input has AGENT_VIEW + AI_ENABLED + AMBIENT_AGENT_V2 set.
    let session =
        Availability::AGENT_VIEW | Availability::AI_ENABLED | Availability::AMBIENT_AGENT_V2;
    assert!(session.contains(command));
}
...
#[test]
fn ambient_agent_v2_required_command_not_satisfied_outside_v2() {
    let command =
        Availability::AGENT_VIEW | Availability::AI_ENABLED | Availability::AMBIENT_AGENT_V2;
    ...
    // Local-mode agent view (NOT_AMBIENT_AGENT instead of AMBIENT_AGENT_V2) must not match either.
    let session =
        Availability::AGENT_VIEW | Availability::AI_ENABLED | Availability::NOT_AMBIENT_AGENT;
    assert!(!session.contains(command));
}
