...
use crate::interaction_sources::{
    AddTabWithShellSource, LaunchConfigUiLocation, SharingDialogSource,
};
...
use crate::settings::{
    ...
    PaneSettings, SelectionSettings, SshSettings, ThemeSettings, active_theme_kind,
...
use warp_core::ui::theme::{AnsiColors, Fill, color::internal_colors, phenomenon::PhenomenonStyle};
...
fn new_session_menu_available_height(window_height: f32, anchor_y: f32) -> f32 {
    (window_height - anchor_y - NEW_SESSION_MENU_WINDOW_MARGIN - NEW_SESSION_MENU_CHROME_HEIGHT)
        .max(NEW_SESSION_MENU_MIN_HEIGHT)
}

/// Shared position ID for the move-to-group sidecar overlay, used by both the
/// `SavePosition` wrapper and the safe-zone rect lookup on the parent menu.
...
impl WorkspaceBanner {
    /// We want some banners to have a close button and not others.
...
pub struct Workspace {
    ...
    /// Sidecar menu rendered next to tab group "Move to group" menu parents.
...
impl Workspace {
    ...
    pub(crate) fn set_is_tab_being_dragged(&mut self, value: bool) {
        self.current_workspace_state.is_tab_being_dragged = value;
    }
    ...
    pub fn handle_tab_group_rename_editor_event(
            ...
            .is_any_tab_group_being_renamed()
        {
            match event {
                ...
                // Losing focus must not persist an unfinished group name.
    ...
    fn configure_empty_workspace(
        ...
        if open_warp_drive {
            ...
            // New local users can still have non-welcome objects from imports or direct invitations.
    ...
    /// Opens a locally indexed conversation by its historical server token.
    pub fn open_cloud_conversation_from_server_token(
        ...
        let Some(_conversation_id) = history.find_conversation_id_by_server_token(&server_token)
        else {
    ...
    /// Resolve a historical server token to local conversation data.
    pub fn load_cloud_conversation_into_new_transcript_viewer(
        &mut self,
        conversation_id: ServerConversationToken,
        ctx: &mut ViewContext<Self>,
    ) {
        if let Some(local_conversation_id) = BlocklistAIHistoryModel::as_ref(ctx)
            .find_conversation_id_by_server_token(&conversation_id)
        {
            self.restore_conversation_in_new_tab(local_conversation_id, ctx);
            return;
        }

        log::warn!(
            "Hosted conversation transcript {} is unavailable in this local-first build",
            conversation_id.as_str()
        );
        self.toast_stack.update(ctx, |view, ctx| {
            let new_toast = DismissibleToast::error(
                "Hosted conversation transcripts are unavailable in this local-first build."
                    .to_string(),
            );
    ...
    fn open_share_session_modal(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        let _ = (index, ctx);
        log::info!("Ignoring hosted shared-session share action in local-first build");
    }
    ...
    fn copy_shared_session_link_from_tab(&mut self, tab_index: usize, ctx: &mut ViewContext<Self>) {
        let _ = (tab_index, ctx);
        log::info!("Ignoring hosted shared-session link copy in local-first build");
    }
    ...
    fn subscribe_to_shared_session_manager(ctx: &mut ViewContext<Self>) {
        ...
        ctx.subscribe_to_model(&manager, move |_me, _, event, ctx| {
            match event {
    ...
    ///
    /// Order: Agent → Terminal (sidecar) → Agent Workspace → [tab configs] → separator → New worktree config (sidecar) → New tab config → separator → Reopen closed session.
    ...
    fn new_session_menu_max_height(
        ...
        new_session_menu_available_height(window.size().y(), anchor_y)
    }
    ...
    pub fn open_or_toggle_warp_drive(
        ...
        // Diagnostics and welcome tip logic is only for when the user explicitly opens Warp Drive
    ...
    fn close_tab(
        ...
        // Diagnostics whenever tabs actually closed, not when confirmation dialog comes up.
        if tabs_closed {
            ctx.dispatch_global_action("workspace:save_app", ());
        }
    }
        ...
        match event {
            ...
            pane_group::Event::OpenWarpDriveLink {
                ...
                match open_warp_drive_args.object_type {
                    ...
                    _ => {
                        log::warn!("Attempted to open an unsupported local-object link")
                    }
                }
            }
            #[cfg(feature = "local_fs")]
                ...
                self.toast_stack
                    .update(ctx, |view, ctx| match result.success_type {
                        OperationSuccessType::Success => {
                            ...
                            {
                                // This is a plan. Only show the save toast if the plan is open in
            ...
            if vertical_tabs_active {
                ...
                let tooltip = if self.left_panel_views.len() <= 1 {
                        ...
                        .unwrap_or(ToolPanelView::ProjectExplorer)
                    {
                        ...
                        ToolPanelView::WarpDrive => "Local objects",
    ...
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            // Hosted download redirects are disabled in this local-first build.
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            ExportAllWarpDriveObjects => {
                // Hosted drive export is disabled in this local-first build.
            }
            ...
            DropTab => {
                let _is_cross_window = CrossWindowTabDrag::as_ref(ctx).is_active();
                let _handed_off_tab_index = CrossWindowTabDrag::as_ref(ctx)
                    .handed_off_target()
                    .map(|_| {
            ...
            Reauth => {
                log::info!("Skipping remote reauth in local workflow");
            }
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(menu_anchor) = self.show_new_session_dropdown_menu {
            ...
            // Action sidecar for actionable items (Terminal, Agent, Agent Workspace, tab configs).
        ...
        if groups_enabled {
            ...
            if expanded_target != source_group {
                ...
                if let Some(target_gid) = expanded_target
                    && let Some((first, last)) = group_member_index_range(&self.tabs, target_gid)
                {
...
fn render_group_member_icon_collage(
    ...
    let scale = total_size / GROUP_ICON_COLLAGE_SIZE;
    let mini_size = GROUP_ICON_COLLAGE_MINI_SIZE * scale;
    let center_inset = GROUP_ICON_COLLAGE_CENTER_INSET * scale;
    let radius = total_size / 2.0 - center_inset;
    ...
    for (idx, kind) in kinds.iter().take(count).enumerate() {
        let mini = render_summary_pane_kind_icon_circle(kind.clone(), mini_size, appearance);
...
/// `creation_order_id`, and takes the first `max_count`.
