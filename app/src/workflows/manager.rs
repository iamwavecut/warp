...
impl WorkflowManager {
    ...
    pub fn create_pane(
        ...
        match source {
            ...
            WorkflowOpenSource::Local { id, .. } => {
                view.update(ctx, |view, ctx| {
                    view.load_local_saved_prompt_by_id(*id, mode, ctx);
                });
            }
            WorkflowOpenSource::NewLocalAgentMode { workflow } => view.update(ctx, |view, ctx| {
                view.open_new_local_saved_prompt(*workflow.clone(), ctx)
            }),
            WorkflowOpenSource::New {
                title,
                content,
