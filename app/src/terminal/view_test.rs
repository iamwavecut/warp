...
use crate::ai::blocklist::{
    ...
    ResponseStreamId, agent_view::AgentViewEntryOrigin,
};
...
use crate::terminal::view::CONTROL_MASTER_BANNER_SUPPRESSED_KEY;
...
#[test]
fn is_passive_conversation_does_not_rederive_from_history_after_construction() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
...
fn copy_does_not_forward_on_normal_screen() {
    App::test((), |mut app| async move {
        ...
        assert!(
            pty_writes.borrow().is_empty(),
            "Copy must not write anything to the PTY on the normal screen"
        );
    })
}
