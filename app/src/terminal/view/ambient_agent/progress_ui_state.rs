...
/// This keeps all loading/error UI handles together and separates them from the main TerminalView.
pub struct AmbientAgentProgressUIState {
    /// State handle for the shimmering text animation in the agent loading screen.
    pub loading_shimmer_handle: ShimmeringTextStateHandle,

    /// Model for displaying tips in the agent loading screen (with 60s cooldown).
    pub tip_model: ModelHandle<AITipModel<AgentLoadingTip>>,

    /// Selection handle for making error text selectable in the agent error screen.
    pub error_selection_handle: SelectionHandle,

    /// Stores selected text from the agent error screen for copying.
...
impl AmbientAgentProgressUIState {
    ...
    pub fn new(ctx: &mut warpui::ModelContext<AmbientAgentViewModel>) -> Self {
        let tip_model = ctx.add_model(|_ctx| {
            ...
            AITipModel::new(ambient_agent::get_agent_loading_tips())
        });
