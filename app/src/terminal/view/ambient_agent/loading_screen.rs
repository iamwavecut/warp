...
pub fn render_cloud_mode_cancelled_screen(appearance: &Appearance) -> Box<dyn Element> {
    ...
    // Title text - "Agent workspace run cancelled"
    let title_text = Text::new(
        "Agent workspace run cancelled",
        appearance.ui_font_family(),
        appearance.monospace_font_size() + 2.,
    )
    .with_style(Properties::default().weight(Weight::Bold))
    ...
    // Subtitle text - "No local agent workspace was started"
    let subtitle_text = Text::new(
        "No local agent workspace was started",
        appearance.ui_font_family(),
        appearance.monospace_font_size(),
    )
