...
const TRANSITION_DIAGNOSTIC_INTERVAL: Duration = Duration::from_secs(60);
...
#[derive(Default)]
pub(super) struct LifecycleDiagnosticsLimiter {
    transitions: HashMap<TransitionKey, RateLimitState>,
}

impl LifecycleDiagnosticsLimiter {
    ...
    pub(super) fn record_at(
        ...
        if now.duration_since(state.last_emitted) < TRANSITION_DIAGNOSTIC_INTERVAL {
