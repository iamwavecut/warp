...
//! policy for an action, and writes local rate-limited diagnostics when the action is conservative
//! or corrective. Callers apply the planned action before committing its next phase.

mod diagnostics;
mod transition;

use diagnostics::LifecycleDiagnosticsLimiter;
pub(in crate::terminal) use diagnostics::LifecycleRecoveryRecord;
...
pub(super) struct BlockLifecycleCoordinator {
    ...
    epoch: u64,
    diagnostics_limiter: LifecycleDiagnosticsLimiter,
}
...
impl Default for BlockLifecycleCoordinator {
    fn default() -> Self {
        Self {
            ...
            epoch: 0,
            diagnostics_limiter: LifecycleDiagnosticsLimiter::default(),
        }
    }
}
...
impl BlockLifecycleCoordinator {
    ...
    pub(super) fn plan(
        ...
        let recovery_record = should_record
            ...
            .and_then(|record| self.diagnostics_limiter.record(record));
