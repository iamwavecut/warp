...
impl ansi::Handler for GridHandler {
    ...
    fn input(&mut self, c: char) {
        ...
        // `not(feature = "test-util")` covers external tests that build real
        // terminal models against `warp`'s test helpers.
        ...
        if width == 0 {
                ...
                && self.ansi_handler_state.supports_emoji_presentation_selector
            {
                ...
                self.update_cursor(|cursor| cursor.point.col = col);
    ...
    fn write_wide_char(&mut self, c: char, update_wide_cell: impl FnOnce(&mut Cell)) {
        ...
        if self.grid.cursor().point.col + 1 >= num_cols {
            if !self.ansi_handler_state.mode.contains(TermMode::LINE_WRAP) {
                self.move_cursor_forward(|cursor| cursor.input_needs_wrap = true);
                return;
            }
        ...
        self.move_cursor_forward(|cursor| cursor.point.col += 1);
