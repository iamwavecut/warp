...
use super::{
    HOOK_JSON_FILE, MINIMUM_PLUGIN_VERSION, VERSION_FILE, check_installed, install_plugin_files_at,
    needs_update_at,
};
...
fn install_writes_current_plugin_files_with_executable_script() {
    ...
    install_plugin_files_at(&dir.path().join("hooks")).expect("install should succeed");

    let hooks = dir.path().join("hooks");
    assert!(check_installed(&dir.path().join("hooks")));
    assert!(!needs_update_at(&dir.path().join("hooks")));
    assert_eq!(
...
fn installed_lifecycle_commands_emit_events_and_preserve_escaped_metadata() {
    let dir = tempfile::tempdir().unwrap();
    install_plugin_files_at(&dir.path().join("hooks")).unwrap();
    let hooks = dir.path().join("hooks");
    let hook_config: Value =
        serde_json::from_slice(&std::fs::read(hooks.join(HOOK_JSON_FILE)).unwrap()).unwrap();
