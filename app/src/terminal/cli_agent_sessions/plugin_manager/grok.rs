...
fn install_plugin_files() -> Result<(), PluginInstallError> {
    ...
    install_plugin_files_at(&hooks_dir)
}

fn install_plugin_files_at(hooks_dir: &Path) -> Result<(), PluginInstallError> {
    let mut log = String::new();
    log.push_str(&format!(
...
fn needs_update_at(hooks_dir: &Path) -> bool {
