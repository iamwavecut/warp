...
/// that this is hard-coded for the default Dark theme. Do not use this for Views
...
pub struct RootView {
    ...
    window_id: WindowId,
    /// Stores the tutorial from onboarding until the workspace exists.
...
impl RootView {
    ...
    fn create_agent_onboarding_view(
        ...
        let onboarding_view = ctx.add_typed_action_view(move |ctx| {
            ...
            let (models, default_model_id) = build_onboarding_models(llm_preferences, ctx);
        ...
        // Subscribe to workspace changes to update autonomy enforcement state.
    ...
    fn handle_agent_onboarding_event(
        ...
        match event {
            ...
            AgentOnboardingEvent::OnboardingCompleted(selected_settings) => {
                ...
                let _onboarding_view = onboarding_view.clone();
            ...
            AgentOnboardingEvent::AppBecameActive => {
                // Refresh local model/workspace data when the user returns to onboarding.
                LLMPreferences::handle(ctx).update(ctx, |prefs, ctx| {
                    prefs.refresh_available_models(ctx);
                });
    ...
    fn handle_incoming_auth_url(&mut self, url: &Url, ctx: &mut ViewContext<Self>) -> bool {
        let _ = (url, ctx);
        log::info!("Ignoring incoming remote auth URL in local workflow");
        true
    }
    ...
    /// Starts a pending onboarding tutorial once the workspace exists.
...
impl AuthOnboardingTarget {
    fn to_workspace(&self, _ctx: &mut ViewContext<RootView>) -> ViewHandle<Workspace> {
        match self {
