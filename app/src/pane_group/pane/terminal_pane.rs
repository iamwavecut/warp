...
use crate::{
    ...
    pane_group::child_agent::{
        ...
        HiddenChildAgentConversationRequest, create_error_child_agent_conversation,
    ...
    workspace::{PaneViewLocator, WorkspaceRegistry, sync_inputs::SyncedInputState},
};
...
impl PaneContent for TerminalPane {
    ...
    fn shareable_link(
        ...
        if lock.is_conversation_transcript_viewer() {
            // Hosted conversation links are disabled in the local-first build.
            ...
            SharedSessionStatus::ActiveViewer { role: _ } => {
                ...
                if let Some(url) = retrieve_shared_session_link(manager, &terminal_view_id) {
                    ...
                    Err(ShareableLinkError::Unexpected(String::from(
                        "Hosted shared session links are disabled",
                    )))
                }
            }
...
fn handle_terminal_view_event(
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
            ...
            Event::OpenWorkflowModalWithLocalWorkflow { id, workflow } => {
            ...
            Event::OpenShareSessionModal { open_source } => {
                ...
                log::info!("Ignoring hosted shared-session modal in local-first build");
            }
            Event::OpenShareSessionDeniedModal => {
                let _ = (terminal_pane_id, ctx);
                log::info!("Ignoring hosted shared-session denied modal in local-first build");
            }
            Event::FocusSession => {
