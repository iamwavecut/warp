...
#[test]
fn local_saved_prompt_pane_snapshot_round_trips_uuid_for_restart() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("warp.sqlite");
    let mut conn = setup_database(&database_path).expect("database should initialize");
        ...
        windows: vec![WindowSnapshot {
            tabs: vec![TabSnapshot {
                ...
                root: PaneNodeSnapshot::Leaf(LeafSnapshot {
                    is_focused: true,
                    custom_vertical_tabs_title: None,
                    contents: LeafContents::Workflow(WorkflowPaneSnapshot::LocalSavedPrompt {
                        prompt_id,
    ...
    let PaneNodeSnapshot::Leaf(LeafSnapshot {
        contents:
            LeafContents::Workflow(WorkflowPaneSnapshot::LocalSavedPrompt {
                prompt_id: restored_id,
                ..
            }),
        ..
    }) = &restored.windows[0].tabs[0].root
    else {
        panic!("Expected local saved prompt pane snapshot");
    };
    assert_eq!(*restored_id, prompt_id);
}
