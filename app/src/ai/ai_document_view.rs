...
use crate::drive::{CloudObjectTypeAndId, items::WarpDriveItemId};
...
impl AIDocumentView {
    pub fn new(
        ...
        // Create save button mouse state for locally saved plans.
        ...
        match save_status {
            AIDocumentSaveStatus::NotSaved => {
                ...
                let tooltip = ui_builder
                    .tool_tip("Save this plan in the local library".to_string())
                    .build()
                    .finish();
            ...
            AIDocumentSaveStatus::Saved => {
                ...
                let ui_builder = appearance.ui_builder().clone();
                let tooltip_text =
                    "This plan is saved locally and will auto save any edits you make.".to_string();
    ...
    fn create_warp_drive_notebook(&self, ctx: &mut ViewContext<Self>) {
        ...
        if !success {
            log::error!("Failed to save local plan notebook");
        }
    }
