...
use crate::ai::execution_profiles::{
    AIExecutionProfile, has_configurable_context_window, should_show_long_context_pricing_warning,
};
use crate::ai::llms::{
...
fn assert_context_window_limit_for_request(
    ...
    App::test((), move |mut app| async move {
        ...
        llm_preferences.update(&mut app, move |preferences, ctx| {
            let available = AvailableLLMs::new(available_model_id, [model], None)
                .expect("test model should create available LLMs");
            preferences.update_models_for_testing(
                ModelsByFeature {
