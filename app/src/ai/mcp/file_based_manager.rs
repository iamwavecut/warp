...
impl FileBasedMCPManager {
    ...
    fn handle_agent_environment_scan_complete(
        ...
        let detected_servers = self
            ...
            .get(repo_path)
            .into_iter()
            .flat_map(|provider_map| provider_map.iter())
            .flat_map(|(provider, hashes)| {
                hashes.iter().filter_map(|hash| {
                    self.file_based_servers.get(hash).map(|installation| {
                        let decision = self.auto_start_decision(*hash, mcp_enabled);
                        AgentEnvMcpScanServer {
                            uuid: installation.uuid(),
                            name: installation.templatable_mcp_server().name.clone(),
                            provider: *provider,
                            hash: *hash,
                            auto_start_eligible: decision.should_autostart,
                        }
                    })
                })
            })
            .sorted_by_key(|server| server.uuid.to_string())
            .collect();
        // FileMCPWatcher emits AgentEnvMcpScanComplete only after emitting ConfigParsed
...
#[derive(Clone, Debug)]
#[allow(dead_code)]
pub struct AgentEnvMcpScanServer {
    pub uuid: Uuid,
    pub name: String,
