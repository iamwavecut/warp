...
fn local_test_installation(
    ...
    TemplatableMCPServerInstallation::new(
        ...
        TemplatableMCPServer {
            uuid: template_uuid,
            name: format!("local-{marker}"),
            description: None,
            template: JsonTemplate {
                json: format!(r#"{{"local-{marker}":{{"command":"echo"}}}}"#),
                variables: Vec::new(),
            },
            version: 0,
            gallery_data: None,
...
fn reconnectable_installation_falls_back_to_ephemeral_state() {
    ...
    let installation = local_test_installation(installation_uuid, Uuid::new_v4(), "ephemeral");
    ...
    assert!(resolved.template_json().contains("local-ephemeral"));
}
...
fn reconnectable_installation_prefers_persisted_state() {
    ...
    manager.reconnectable_ephemeral_installations.insert(
        installation_uuid,
        local_test_installation(installation_uuid, template_uuid, "ephemeral"),
    );
    manager.locally_installed_servers.insert(
        installation_uuid,
        local_test_installation(installation_uuid, template_uuid, "persisted"),
    );
    let resolved = manager
        .reconnectable_installation(installation_uuid)
        .expect("persisted installation should be reconnectable");
    assert!(resolved.template_json().contains("local-persisted"));
}
