...
fn parsed_skill(path: PathBuf, name: &str) -> ParsedSkill {
    let skill_path = LocalOrRemotePath::Local(path);
    let provider = get_provider_for_path(&skill_path).unwrap_or(SkillProvider::Agents);
    ParsedSkill {
        path: skill_path,
        name: name.to_string(),
...
fn skill_paths(skills: Vec<ParsedSkill>) -> Vec<PathBuf> {
    skills
        .into_iter()
        .filter_map(|skill| skill.path.to_local_path().map(Path::to_path_buf))
        .collect()
}
