...
impl SkillManager {
    ...
    pub fn get_skills_for_working_directory(
        ...
        self.get_skills_for_working_location(working_directory.as_ref(), ctx)
    }

    pub fn get_skills_for_working_location(
        &self,
        ...
        let path_matches_location = |path: &LocalOrRemotePath| match (&working_directory, path) {
            ...
            (None, LocalOrRemotePath::Local(_)) => self.is_agent_environment,
        ...
        if self.is_agent_environment {
            // In agent environments, all skills in the active location are in
