...
impl AgentDriver {
    ...
    fn subscribe_to_mcp_managers(
        ...
        ctx.subscribe_to_model(
            ...
            move |_me, manager, event, ctx| match event {
            ...
            if let FileBasedMCPManagerEvent::AgentEnvMcpScanComplete {
                ...
                && pending_repos.remove(repo_path) {
                    ...
                    if pending_repos.is_empty() {
                        ...
                        if let Some(sender) = tx.take() {
                            log::info!(
                                "Collected {} auto-started file-based MCP server(s) from agent environment repos",
                                uuids.len()
                            );
    ...
    /// enabling local notification integration, if applicable.
    ///
