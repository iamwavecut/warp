...
/// Session discovery is best-effort; an already known session must still be checkpointed.
pub(crate) async fn save_after_session_update(
    update: impl Future<Output = Result<()>>,
    save: impl Future<Output = Result<()>>,
) -> Result<()> {
    if let Err(error) = update.await {
        log::warn!("Failed to refresh local harness session before saving: {error:#}");
    }
    save.await
}
...
impl SaveCoordinator {
            ...
            .spawn(async move {
                let worker = async {
                    loop {
                        ...
                        if let Err(error) = worker_operation(save_point).await {
                            log::warn!("Failed to save local harness state: {error:#}");
                        }
                    }
                };
    ...
    /// The driver owns finalization; callers must not run multiple finalizers concurrently.
    pub(crate) async fn finish(
        ...
        if let Some(mut active) = active {
            // A stalled ordinary save must leave time for the post-exit capture.
            if (&mut active.done).with_timeout(budget / 2).await.is_err() {
                active.abort.abort();
                (&mut active.done)
                    .with_timeout(deadline.saturating_duration_since(Instant::now()))
                    .await
                    .map_err(|_| anyhow!("Timed out cancelling the harness save"))?
        ...
        let result = final_save
            ...
            .await
            .map_err(|_| anyhow!("Harness final save timed out"))
